...
export function cardPositions(N) {
    ...
    var degx =30;
    for(var i=0; i<N; i++) {
        Pos[i] = {x : ((N-1)/2-i)*dx + middle, deg : ((N-1)/2-i)*dx/degx, scale:0.75};
    }
    if(N%2) {
        Pos[(N-1)/2].y = y0;
    }
    ...
    for(var i=Math.floor(N/2-1.25); i>=0; i--) {
        Pos[i].y = getRightCardPosition(Pos[i+1], dx, degx);
        Pos[N-i-1].y = getLeftCardPosition(Pos[N-i-2], dx, degx);
    }
    ...
    i0 = hand.findIndex(elem => elem === card);
    
    if(i0 == -1) return Pos;
    Pos[i0].x = Pos[i0].x - Card.W/2 + dx/2;
    Pos[i0].y = document.body.clientHeight - Card.H/2 + 1;
    Pos[i0].deg = 0;
    Pos[i0].scale = 1;
    if(i0<N-1 ) {
        var dx1, dx2, rad;
        rad = Pos[i0+1].deg / 180 * Math.PI;
        dx1 = Card.W / 2 * Math.cos(rad) + Card.H / 2 * Math.sin(rad);
        dx2 = Card.W / 2 * Math.cos(rad);
    ...
    if( i0>0 ) {
        ...
        dx1 = -Card.W / 2 * Math.cos(rad) + Card.H / 2 * Math.sin(rad);
        dx2 = -Card.W / 2 * Math.cos(rad);
        Pos[i0-1].x = Math.max( Pos[i0].x + Card.W/2 - Math.max(dx1, dx2) - 10, Pos[i0-1].x );
 ...
 * @param {number} degx
 */
function getLeftCardPosition(pos, dx, degx) {
    var x,y,deg,dx1,dx2,dy1,dy2,rad;
    x = pos.x-dx;
    deg = pos.deg - dx/degx;
    rad = pos.deg / 180 * Math.PI;
    dx1 = Card.W / 2 * Math.cos(rad) + Card.H / 2 * Math.sin(rad);
    dy1 = Card.W / 2 * Math.sin(rad) - Card.H / 2 * Math.cos(rad);
    rad = deg / 180 * Math.PI;
    dx2 = Card.W / 2 * Math.cos(rad) + Card.H / 2 * Math.sin(rad);
    dy2 = Card.W / 2 * Math.sin(rad) - Card.H / 2 * Math.cos(rad);
 ...
 * @param {number} degx
 */
function getRightCardPosition(pos, dx, degx) {
    var x,y,deg,dx1,dx2,dy1,dy2,rad;
    x = pos.x+dx;
    deg = pos.deg + dx/degx;
    rad = pos.deg / 180 * Math.PI;
